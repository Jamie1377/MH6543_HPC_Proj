...
def process_stock_file(file_path):
    ...
    try:
        ...
        df = pd.read_csv(
            file_path, skiprows=3, names=col_names, index_col=0, parse_dates=True
        )
        ...
        if df.index.hasnans:
            print(
                f"NaT values found in index after conversion for {file_path}, skipping file."
            )
            return None, 0
...
def main():
    ...
    # Get all stock files
    stock_files = [
        f"stock_data/{f}" for f in os.listdir("stock_data") if f.endswith(".csv")
    ]
    ...
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        for result, rows in tqdm(
            executor.map(process_stock_file, stock_files), total=len(stock_files)
        ):
            if result:
