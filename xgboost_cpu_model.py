...
def load_data(file_path="processed_data/AAPL_processed.csv", sample_frac=1.0):
...
def tune_hyperparameters_cpu(X_train, y_train, search_method="random"):
    ...
    print(
        f"Starting hyperparameter tuning with {'RandomizedSearchCV' if search_method == 'random' else 'GridSearchCV'}..."
    )
    ...
    # Want to keep the grid larger for CPU to optmize performance
    param_grid = {
        "max_depth": [3, 6, 9],
...
def main():
    ...
    best_params = tune_hyperparameters_cpu(X_train, y_train, search_method="grid")

    # Train with tuned parameters
    print("\n--- Training with CPU using tuned parameters ---")
    model_cpu, time_cpu = train_model_cpu(
        X_train, y_train, X_test, y_test, best_params=best_params
    )
